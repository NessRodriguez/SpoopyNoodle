...
class App:
    ...
    def on_init(self):
        ...
        self._running = True
        self._image_surf = pygame.image.load("gollum.jpg").convert()
        self._apple_surf = pygame.image.load("frodo.jpg").convert()
    ...
    def on_loop(self):
        ...
        for i in range (1,self.computer.length):
            if self.game.isCollision(self.apple.x,self.apple.y,self.computer.x[i],self.computer.y[i],42):
                self.apple.x = randint(2,9) * 44
                self.apple.y = randint(2,9) * 44
